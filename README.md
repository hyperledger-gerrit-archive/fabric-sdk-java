 ...
 * Allow vagrant to navigate to your SDK project directory (assuming you have the SDK under the _src/github.com/hyperledger_ tree like the other hyperledger projects)
```
 ...
 * Start the needed fabric services in vagrant.  In the vagrant system:
   1. _cd $GOPATH/src/github.com/hyperledger/fabric-sdk-java/src/test/fixture/src_
 ...
 * _src/test/fixture/e2e-2Orgs/channel_  (everything needed to bootstrap the orderer and create the channels)
 * _src/test/fixture/e2e-2Orgs/crypto-config_ (as-is. Used by `configtxgen` and `docker-compose` to map the MSP directories)
 * _src/test/fixture/src/docker-compose.yaml_
...
The end to end test case artifacts are stored under in directory _src/test/fixture/e2e-2Org/crypto-config_ .
...
and one file in th _src/test/fixture/e2e-2Orgs/channel_ directory specifically for use in the end to end test scenario
...
Channel configuration files and orderer bootstrap files ( see directory _src/test/fixture/e2e-2Orgs/channel_ ) are needed when creating a new channel.
...
with the configtxgen config file _src/test/fixture/e2e-2Orgs/channel/configtx.yaml_
