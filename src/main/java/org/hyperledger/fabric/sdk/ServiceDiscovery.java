...
class ServiceDiscovery {
    ...
    public SDChaindcode discoverEndorserEndpoint(TransactionContext transactionContext, final String name) {
        ...
        return dchaindcodeMap.get(name);
    }
    ...
    class SDNetwork {
        ...
        public List<SDEndorser> getEndorers() {
            return endorers;
        }

        List<SDEndorser> endorers;
        Set<String> peerEndpoints = Collections.emptySet();
        Set<SDOrderer> ordererEndpoints = Collections.emptySet();

        Set<SDOrderer> getOrdererEndpoints() {
    ...
    public SDNetwork networkDiscovery(TransactionContext ltransactionContext) {

        ArrayList<Peer> speers = new ArrayList<>(serviceDiscoveryPeers);
        Collections.shuffle(speers);
        SDNetwork ret = sdNetwork;

        if (null != ret && ret.discoveryTime + SERVICE_DISCOVER_FREQ_SECONDS * 1000 > System.currentTimeMillis()) {
        ...
        for (Peer serviceDiscoveryPeer : speers) {
            ...
            try {
                ...
                Set<SDOrderer> ordererEndpoints = new HashSet<>();
                Map<String, Protocol.Endpoints> orderersMap = configResult.getOrderersMap();
                for (Map.Entry<String, Protocol.Endpoints> i : orderersMap.entrySet()) {
                    String key = i.getKey();
                    String mspid = msps.get(key).getName();

                    Protocol.Endpoints value = i.getValue();
                    for (Protocol.Endpoint l : value.getEndpointList()) {
                        String endpoint = l.getHost() + ":" + l.getPort();

                        ordererEndpoints.add(new SDOrderer(mspid, endpoint.trim().toLowerCase(), lsdNetwork.getTlsCerts(mspid), lsdNetwork.getTlsIntermediateCerts(mspid)));
                    }
                }
                ...
                lsdNetwork.endorers = new ArrayList<>();
                ...
                lsdNetwork.peerEndpoints = peerEndpoints;
    ...
    Map<String, SDChaindcode> discoverEndorserEndpoints(TransactionContext transactionContext, Set<String> chaincodeNames) {
        ...
        for (Peer serviceDiscoveryPeer : speers) {
            ...
            try {
                ...
                resultsList.forEach(queryResult -> {
    ...
    void run() {
        ...
        if (seviceDiscovery == null) {
            ...
            seviceDiscovery = Executors.newSingleThreadScheduledExecutor(r -> {
                ...
                try {
                    SDNetwork sdNetwork = networkDiscovery(transactionContext.retryTransactionSameContext());
                    ...
                    channel.sdUpdate(serviceDiscoveryPeers.iterator().next(), sdNetwork);
