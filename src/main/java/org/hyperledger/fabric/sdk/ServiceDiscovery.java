...
class ServiceDiscovery {
    ...
    private final ConcurrentSet<byte[]> certs = new ConcurrentSet<>();
        ...
        for (Peer serviceDiscoveryPeer : speers) {
            ...
            try {
                ...
                for (Map.Entry<String, MspConfig.FabricMSPConfig> i : msps.entrySet()) {
                    ...
                    value.getRootCertsList().forEach(bytes -> cb.add(bytes.toByteArray()));
                    value.getIntermediateCertsList().forEach(bytes -> cb.add(bytes.toByteArray()));
