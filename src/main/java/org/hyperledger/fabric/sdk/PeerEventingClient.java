...
//    }
                        ...
                        try {
                            ...
                            if (null != disconnectedHandler) {
                                try {
                                    ...
                                    logger.warn(format("Eventhub %s  %s", PeerEventingClient.this.name, e.getMessage()), e);
                                    peer.getChannel().getChannelEventQue().eventError(e);
                                }
                            }
                        } catch (Exception e) {
                            logger.warn(format("Eventhub %s Failed shutdown msg:  %s", PeerEventingClient.this.name, e.getMessage()), e);
                        }
                    }
                }
    ...
    private void blockListen(TransactionContext transactionContext) throws CryptoException {
        ...
        PeerEvents.RegisterChannel register = PeerEvents.RegisterChannel.newBuilder().addAllChannelIds(channelNames).build();
        ...
        if (logger.isDebugEnabled()) {
            ...
            logger.debug("Registering for channels: " + sb.toString());
        }
