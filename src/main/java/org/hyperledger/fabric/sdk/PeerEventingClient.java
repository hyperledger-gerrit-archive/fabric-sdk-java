...
import org.hyperledger.fabric.protos.peer.PeerEvents.ChannelServiceRequest;
import org.hyperledger.fabric.protos.peer.PeerEvents.ChannelServiceResponse;
import org.hyperledger.fabric.protos.peer.PeerEvents.RegisterChannel;
...
public class PeerEventingClient implements Serializable {
    ...
    private transient StreamObserver<Envelope> sender;
...
//    }
    ...
    private transient StreamObserver<ChannelServiceResponse> eventStream = null; // Saved here to avoid potential garbage collection
    ...
    synchronized boolean connect(final TransactionContext transactionContext) throws EventHubException {
        ...
        logger.debug(format("Peer eventing service %s is connecting to url %s:", name, url));
        ...
        final StreamObserver<ChannelServiceResponse> eventStreamLocal = new StreamObserver<ChannelServiceResponse>() {
            @Override
            public void onNext(final ChannelServiceResponse channelServiceResponse) {

                final ChannelServiceResponse.ResponseCase responseCase = channelServiceResponse.getResponseCase();
                if (ChannelServiceResponse.ResponseCase.RESULT.equals(responseCase)) {

                    final PeerEvents.ChannelServiceResult channelServiceResult = channelServiceResponse.getResult();

                    final String action = channelServiceResult.getAction();
                    logger.debug("action:" + action);

                    if (channelServiceResult.getSuccess()) { // all or nothing.
                        connected = true;
                        ...
                        for (PeerEvents.ChannelResult result : channelServiceResult.getChannelResultsList()) {
                ...
                } else if (ChannelServiceResponse.ResponseCase.EVENT.equals(responseCase)) {
                    final PeerEvents.Event event = channelServiceResponse.getEvent();

                    try {
                        logger.trace(format("Peer eventing adding event %s to channel queue block number %d",
                                peer.getName(), event.getBlock().getHeader().getNumber()));
                    ...
                    final EventHubException eventHubException = new EventHubException(format("Peer %s eventing service error unexpected case for response number: %d",
                            name, responseCase.getNumber()));
                    logger.error(eventHubException);
                    finishLatch.countDown();
        ...
        try {
            if (!finishLatch.await(EVENTHUB_CONNECTION_WAIT_TIME, TimeUnit.MILLISECONDS)) {
                EventHubException evh = new EventHubException(format("Peer eventing %s failed to connect in %s ms.", name, EVENTHUB_CONNECTION_WAIT_TIME));
    ...
    private void blockListen(TransactionContext transactionContext) throws CryptoException {
        ...
        RegisterChannel register = RegisterChannel.newBuilder()
        ...
        final ChannelServiceRequest channelServiceRequest = ChannelServiceRequest.newBuilder()
                .setRegisterChannel(register)
                .build();
        sender.onNext(evelopeBuilder(channelServiceRequest));
    }

    Envelope evelopeBuilder(ChannelServiceRequest channelServiceRequest) throws CryptoException {

        ChannelHeader channelHeader = createChannelHeader(Common.HeaderType.CHANNEL_SERVICE_REQUEST,
                transactionContext.getTxID(), name, transactionContext.getEpoch(), getCurrentFabricTimestamp(), null);
        ...
        Payload seekPayload = Payload.newBuilder()
                .setHeader(header)
                .setData(channelServiceRequest.toByteString())
                .build();
