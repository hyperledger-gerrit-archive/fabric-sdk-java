...
public class Chain {
	...
	Collection<ProposalResponse> sendDeploymentProposal(DeploymentProposalRequest deploymentProposalRequest, Collection<Peer> peers) throws Exception {
		...
		for (Peer peer : peers) {// TODO need to make async.
			...
			ProposalResponse proposalResponse = new ProposalResponse(transactionContext.getTxID(), transactionContext.getChainID(), fabricResponse.getResponse().getStatus(), fabricResponse.getResponse().getMessage());
			proposalResponse.setProposalResponse(fabricResponse);
			proposalResponse.setProposal(signedProposal);

			proposalResponse.verify(this.cryptoPrimitives);
	...
	private Collection<ProposalResponse> sendProposal(TransactionRequest queryProposalRequest, Collection<Peer> peers) throws Exception {
		...
		proposalBuilder.ccType(queryProposalRequest.getChaincodeLanguage() == TransactionRequest.Type.JAVA ? Chaincode.ChaincodeSpec.Type.JAVA : Chaincode.ChaincodeSpec.Type.GOLANG);
	...
	CompletableFuture<Envelope> sendTransaction(Collection<ProposalResponse> proposalResponses, Collection<Orderer> orderers) throws InvalidArgumentException, CryptoException, InvalidProtocolBufferException {
		...
		Payload transactionPayload = transactionBuilder.hash(transactionContext.getCryptoPrimitives()::hash).chaincodeProposal(proposal).endorsements(ed).proposalResponcePayload(proposalResponsePayload).build();
		...
		for (Orderer orderer : orderers) {// TODO need to make async.
			...
			// TransactionResponse tresp = new
			// TransactionResponse(transactionContext.getTxID(),
			// transactionContext.getChainID(), resp.getStatusValue(),
			// resp.getStatus().name());
	...
	public class ChainEventQue {
		...
		private final BlockingQueue<Event> events = new LinkedBlockingQueue<>();// Thread
																				// safe
		...
		public boolean addBEvent(Event event) {
			...
			// If being fed by multiple eventhubs make sure we don't add dups
			// here.
			synchronized (this) {
				if (num <= previous) {
				...
				for (ByteString db : data.getDataList()) {
					...
					try {
						...
						for (BL l : blcopy) {
							...
							} catch (Throwable e) { // Don't let one register
													// stop rest.
								logger.error(e);
							}
				...
				try {
					...
					for (TL l : txL) {
						try {
							...
							logger.error(e); // Don't let one register stop
												// rest.
						}
					}
