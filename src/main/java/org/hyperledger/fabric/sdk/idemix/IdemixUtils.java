...
public final class IdemixUtils {
    ...
    public static final ECP genG1 = new ECP(gx, gy);
    ...
    public static final ECP2 genG2 = new ECP2(px, py);
    public static final FP12 genGT = PAIR.fexp(PAIR.ate(genG2, genG1));
    public static final BIG GROUP_ORDER = new BIG(ROM.CURVE_Order);
    public static final int FIELD_BYTES = BIG.MODBYTES;
     ...
     */
    protected static int append(byte[] data, int index, byte[] toAppend) {
     ...
     * @return the new index up to which the data has been filled
     */
    protected static int append(byte[] data, int index, boolean[] toAppend) {
        byte[] toAppendBytes = new byte[toAppend.length];
        for (int i = 0; i < toAppend.length; i++) {
            toAppendBytes[i] = toAppend[i] ? (byte) 1 : (byte) 0;
        }
        return append(data, index, toAppendBytes);
    }
