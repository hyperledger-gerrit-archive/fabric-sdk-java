...
public class IdemixSignature {
    ...
    private int[] hiddenIndices(boolean[] disclosure) {
        ...
        int counter = 0;
        for (boolean aDisclosure : disclosure) {
            if (!aDisclosure) {
                counter++;
            }
        }
     ...
     */
    public IdemixSignature(IdemixCredential c, BIG sk, IdemixPseudonym pseudonym, IdemixIssuerPublicKey ipk, boolean[] disclosure, byte[] msg) {
     ...
     */
    public boolean ver(boolean[] disclosure, IdemixIssuerPublicKey ipk, byte[] msg, BIG[] attributeValues) {
