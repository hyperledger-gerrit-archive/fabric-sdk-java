...
public class IdemixSignature {
    ...
    public IdemixSignature(IdemixCredential c, BIG sk, IdemixPseudonym pseudonym, IdemixIssuerPublicKey ipk, boolean[] disclosure, byte[] msg, int rhIndex, Idemix.CredentialRevocationInformation cri) {
        ...
        byte[] proofData = new byte[SIGN_LABEL.getBytes().length + 7 * (2 * IdemixUtils.FIELD_BYTES + 1) + ipk.getHash().length + disclosure.length + msg.length + RevocationAuthority.getProofBytes(revocationAlgorithm)];
        int index = 0;
        ...
        index = IdemixUtils.append(proofData, index, nonRevokedProofHashData);
        ...
        this.nonRevocationProof = prover.getNonRevokedProof(this.proofC);
    }
    ...
    public boolean verify(boolean[] disclosure, IdemixIssuerPublicKey ipk, byte[] msg, BIG[] attributeValues, int rhIndex, PublicKey revPk, int epoch) throws CryptoException {
        ...
        index = IdemixUtils.append(proofData, index, nonRevokedProofBytes);
        index = IdemixUtils.append(proofData, index, ipk.getHash());
        index = IdemixUtils.append(proofData, index, disclosure);
        IdemixUtils.append(proofData, index, msg);
