...
public class IdemixIssuerPublicKey {
    ...
    public IdemixIssuerPublicKey(String[] attributeNames, BIG isk) {
        ...
        for (int i = 0; i < attributeNames.length; i++) {
            HAttrs[i] = IdemixUtils.genG1.mul(IdemixUtils.randModOrder());
        }

        // Generating Hsk value
        Hsk = IdemixUtils.genG1.mul(IdemixUtils.randModOrder());

        // Generating HRand value
        HRand = IdemixUtils.genG1.mul(IdemixUtils.randModOrder());

        // Generating BarG1 value
        BarG1 = IdemixUtils.genG1.mul(IdemixUtils.randModOrder());
        ...
        BIG r = IdemixUtils.randModOrder();
        ...
        index = IdemixUtils.append(proofData, index, IdemixUtils.ecpToBytes(BarG1));
        index = IdemixUtils.append(proofData, index, IdemixUtils.ecpToBytes(W));
        IdemixUtils.append(proofData, index, IdemixUtils.ecpToBytes(BarG2));
        ...
        // Computing Hash of IdemixIssuerPublicKey
     ...
     * @return an issuer public key created from the proto
     */
    ...
    public boolean check() {
        ...
        if (!Arrays.equals(IdemixUtils.bigToBytes(IdemixUtils.hashModOrder(proofData)), IdemixUtils.bigToBytes(ProofC))) {
            return false;
        }
