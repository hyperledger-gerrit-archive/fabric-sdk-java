...
public class ProposalResponse extends ChainCodeResponse {
	 ...
	 * field
	 * 
	 * @param crypto  the CryptoPrimitives instance to be used for signing and verification
	 * 
	...
	public boolean verify(CryptoPrimitives crypto) {
		...
		try {
			...
			this.isVerified = crypto.verify(plainText.toByteArray(), sig.toByteArray(), endorser.getIdBytes().toByteArray());
