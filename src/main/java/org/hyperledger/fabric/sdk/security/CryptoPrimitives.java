...
public class CryptoPrimitives {
    ...
    public byte[] eciesDecrypt(KeyPair keyPair, byte[] data) throws CryptoException {
        try {
            int ekLen = (int) (Math.floor((this.securityLevel + 7) / 8) * 2 + 1);
            int mkLen = this.securityLevel >> 3;
            int emLen = data.length - ekLen - mkLen;

            byte[] ephemeralPublicKeyBytes = Arrays.copyOfRange(data, 0, ekLen);
            byte[] encryptedMessage = Arrays.copyOfRange(data, ekLen, ekLen + emLen);
            byte[] tag = Arrays.copyOfRange(data, ekLen + emLen, data.length);
