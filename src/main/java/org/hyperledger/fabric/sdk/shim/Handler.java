...
import static org.hyperledger.protos.peer.Chaincode.ChaincodeMessage.Type.*;
...
public class Handler {
     ...
     */
    public void handleInit(Chaincode.ChaincodeMessage message) throws InvalidTransactionException {
        Runnable task = () -> {
            ...
            try {
                ...
                logger.debug(String.format("[%s]Init succeeded. Sending %s",
    ...
    public void handleQuery(Chaincode.ChaincodeMessage message) {
        // Query does not transition state. It can happen anytime after Ready
        Runnable task = () -> {
            Chaincode.ChaincodeMessage serialSendMessage = null;
            try {
                // Get the function and args from Payload
                Chaincode.ChaincodeInput input;
                try {
                    ...
                    serialSendMessage = Chaincode.ChaincodeMessage.newBuilder()
                            .setType(QUERY_ERROR)
                            .setPayload(ByteString.copyFromUtf8(e.getMessage()))
                            .setTxid(message.getTxid())
                            .build();
    ...
    public ByteString handleQueryChaincode(String chaincodeName, String function, List<ByteString> args, String uuid) {
        Chaincode.ChaincodeID id = Chaincode.ChaincodeID.newBuilder().setName(chaincodeName).build();
        Chaincode.ChaincodeInput input = Chaincode.ChaincodeInput.newBuilder()
                .addArgs(ByteString.copyFromUtf8(function))
                .addAllArgs(args)
                .build();
        ...
        try {
            ...
            logger.debug("Another request pending for this Uuid. Cannot process.");
            throw e;
        }
