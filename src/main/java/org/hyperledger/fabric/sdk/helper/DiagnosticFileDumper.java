...
//    static DiagnosticFileDumper getInstance() {
//
//        return one;
//
//    }

    public String createDiagnoticProtobufFile(byte[] byteString) {

        return createDiagnoticFile(byteString, "protobuf_", "proto");
    ...
    public String createDiagnoticFile(byte[] byteString, String prefix, String ext) {
        String ret = "";
        if (!canWrite()) {
        ...
        if (null != byteString) {
            ...
            int localCount = counter.addAndGet(1);

            SimpleDateFormat dateFormatGmt = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss_SSS");
            dateFormatGmt.setTimeZone(TimeZone.getTimeZone("UTC"));
            final String timstamp = dateFormatGmt.format(new Date());
            ret = prefix + timstamp + "_" + localCount + "." + ext;
            ret = ret.replaceAll("\\:", "-");
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                queEntries.add(protos.take());

                if (!canWrite()) {
                    return;  //IF the directoy is missing just assume user does not want diagnosit files created anymore.
                }
