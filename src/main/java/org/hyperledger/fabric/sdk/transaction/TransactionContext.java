...
*/
    ...
    public ChainCodeResponse deploy(DeployRequest deployRequest) throws ChainCodeException {
        ...
        if (null == tcert) {
            ...
            throw new ChainCodeException("Failed getting a new TCert", null);
        }
        ...
        try {
            ...
            throw new ChainCodeException("Failed to deploy transaction", e);
        }
    ...
    public ChainCodeResponse invoke(InvokeRequest invokeRequest) throws ChainCodeException {        
        ...
        try {
            ...
            throw new ChainCodeException("Failed to invoke transaction", e);
        }
    ...
    public ChainCodeResponse query(QueryRequest queryRequest) throws ChainCodeException {
        ...
        try {
            ...
            throw new ChainCodeException("Failed to query transaction", e);
        }
   ...
   public void setAttrs(List<String> attrs) {
     ...
     */
    private String execute(Transaction tx) throws CryptoException, IOException {
        ...
        // Add the tcert
        tx.getTxBuilder().setCert(ByteString.copyFrom((byte[]) tcert.encode()));
        // sign the transaction bytes
        byte[] txBytes = tx.getTxBuilder().buildPartial().toByteArray();
        BigInteger[] signature = this.chain.getCryptoPrimitives().ecdsaSign((PrivateKey) tcert.getPrivateKey(), txBytes);
        ...
        if (tx.getTxBuilder().getConfidentialityLevel() == Chaincode.ConfidentialityLevel.CONFIDENTIAL &&
            ...
            String message = this.getChain().sendTransaction(tx.getTxBuilder().build());
            return new String(this.decryptResult(message.getBytes()));
        } else {
