 ...
 * It contains an (un)linkable pseudonym, revealed attribute values,
 * and a corresponding proof of possession of an Idemix credential
 *
 */
...
/*    	if (proto.getMspid() == null) {
    		throw new CryptoException("MSP ID must not be null");	
    	} 
*/
		...
		if (idemixProto != null) {    	    
	    	this.nym = new ECP(BIG.fromBytes(idemixProto.getNymX().toByteArray()), BIG.fromBytes(idemixProto.getNymY().toByteArray()));
    ...
    public IdemixIdentity(String mspId, ECP nym, byte[] ou, byte[] role, Signature proof) throws CryptoException{
    	...
    	if ((mspId == null) || (mspId == "") || (nym == null) || (ou == null) || (role == null) || (proof == null)){
    		throw new CryptoException("Input must not be null");
    	}
    ...
    public Identities.SerializedIdentity createSerializedIdentity(){
    	Identities.SerializedIdemixIdentity serializedIdemixIdentity =
    		Identities.SerializedIdemixIdentity.newBuilder()
