...
/**
 * FCAClient is the default implementation of a member services client.
 */
public class FCAClient implements MemberServices {
    private static final Log logger = LogFactory.getLog(FCAClient.class);
     ...
     */
    public FCAClient(String url, String pem) throws MalformedURLException {
        this.url = url;

        URL purl = new URL(url);
        final String proto = purl.getProtocol();
        if (!"http".equals(proto) && !"https".equals(proto)) {
            throw new IllegalArgumentException("FCAClient only supports http or https not " + proto);
        }
        final String host = purl.getHost();

        if (StringUtil.isNullOrEmpty(host)) {
            throw new IllegalArgumentException("FCAClient url needs host");
        }
        ...
        if (!StringUtil.isNullOrEmpty(path)) {
            ...
            throw new IllegalArgumentException("FCAClient url does not support path portion in url remove path: '" + path + "'.");
        }
        ...
        if (!StringUtil.isNullOrEmpty(query)) {
            ...
            throw new IllegalArgumentException("FCAClient url does not support query portion in url remove query: '" + query + "'.");
        }
...
//    public Enrollment enroll(String name, String adminpw) {
//    }
     ...
     */
    @Override
    public Enrollment enroll(String user, String secret) throws EnrollmentException {
        ...
        logger.debug("[FCAClient.enroll] Generating keys...");

        try {
            // generate ECDSA keys: signing and encryption keys
            KeyPair signingKeyPair = cryptoPrimitives.keyGen();
            logger.debug("[FCAClient.enroll] Generating keys...done!");
            ...
            logger.debug(format("[FCAClient] enroll success:[%s], result:[%s]", success, result));
            ...
            logger.info(format("[FCAClient] enroll returned pem:[%s]", signedPem));

            Enrollment enrollment = new FCAEnrollment(signingKeyPair, Hex.toHexString(signingKeyPair.getPublic().getEncoded()), signedPem);
            return enrollment;
