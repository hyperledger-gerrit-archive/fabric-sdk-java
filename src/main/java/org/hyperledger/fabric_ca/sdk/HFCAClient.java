...
/*
    ...
    public Enrollment reenroll(User user) throws EnrollmentException, InvalidArgumentException {
        ...
        try {
            ...
            // generate CSR
            PKCS10CertificationRequest csr = cryptoPrimitives.generateCertificationRequest(user.getName(), user.getEnrollment().getKeyPair());
            String pem = cryptoPrimitives.certificationRequestToPEM(csr);
            ...
            return new HFCAEnrollment(user.getEnrollment().getKeyPair(), user.getEnrollment().getPublicKey(), signedPem);
