    ...
    public void setup() {
        ...
        try {
            ...
            chain.sendTransaction(successful, orderers).thenApply(transactionEvent -> {
                ...
                out(String.format("Finished instantiate transaction with transaction id %s", transactionEvent.getTransactionID()));
                ...
                try {
                    ...
                    Assert.assertTrue(transactionEvent.isValid()); // must be valid to be here.
                    out(String.format("Finished invoke transaction with transaction id %s", transactionEvent.getTransactionID()));
                ...
                if (e instanceof TransactionEventException) {
                    ...
                    if (te != null) {
                        ...
                        Assert.fail(String.format("Transaction with txid %s failed", te.getTransactionID()));
                    }
                }
                Assert.fail(String.format("Transaction  failed  %s", e.getMessage()));
                return null;
