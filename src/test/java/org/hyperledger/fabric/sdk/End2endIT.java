...
public class End2endIT {
    ...
    public void setup() {
        ...
        try {
          ...
          //  runChain(client, constructChain(client), true, 0);// Run test chain



            Chain fooChain = constructFooChain(client);
            Peer aPeer = fooChain.getPeers().iterator().next();
            fooChain.joinPeer(aPeer);




            out("\n");
            runChain(client, fooChain, false, 100);//run a newly constructed foo chain with different b value!
