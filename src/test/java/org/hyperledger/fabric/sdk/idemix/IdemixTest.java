...
public class IdemixTest {
    ...
    public void testIdemix() throws CryptoException {
        ...
        IdemixIssuerKey key = new IdemixIssuerKey(attributeNames, rand);
        // check that the issuer public key is valid
        assertTrue(key.getIpk().check());

        // Test serialization of issuer key
        assertTrue(new IdemixIssuerKey(key.toProto()).getIpk().check());

        // Choose a user secret key and request a credential
        BIG sk = new BIG(IdemixUtils.randModOrder(rand));
        BIG issuerNonce = new BIG(IdemixUtils.randModOrder(rand));
        IdemixCredRequest m = new IdemixCredRequest(sk, issuerNonce, key.getIpk(), rand);
        ...
        IdemixCredential c = new IdemixCredential(key, m, attrs, rand);
        ...
        IdemixPseudonym p = new IdemixPseudonym(sk, key.getIpk(), rand);
        ECP nym = p.getNym();
        ...
        byte[] disclosure = {0, 0};
        byte[] msg = {1, 2, 3, 4};
        IdemixSignature sig = new IdemixSignature(c, sk, nym, randNym, key.getIpk(), disclosure, msg, rand);
        ...
        sig = new IdemixSignature(c, sk, nym, randNym, key.getIpk(), disclosure, msg, rand);
        // check that the signature is valid
        assertTrue(sig.ver(disclosure, key.getIpk(), msg, attrs));

        // Sign a message with respect to a pseudonym
        IdemixNymSignature nymsig = new IdemixNymSignature(sk, nym, randNym, key.getIpk(), msg, rand);
        // check that the pseudonym signature is valid
        assertTrue(nymsig.ver(nym, key.getIpk(), msg));

        // Test serialization of IdemixNymSignature
        assertTrue(new IdemixNymSignature(nymsig.toProto()).ver(nym, key.getIpk(), msg));
    }
}
