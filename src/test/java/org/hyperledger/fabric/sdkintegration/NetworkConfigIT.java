...
public class NetworkConfigIT {
    ...
    private static User getAdminUser() throws Exception {
        ...
        String privateKeyFileName = userInfo.getPrivateKeyFile();
        String certificateFileName = userInfo.getSignedCertFile();

        if (privateKeyFileName != null) {
            File file = new File(privateKeyFileName);
            privateKey = getPrivateKeyFromBytes(IOUtils.toByteArray(new FileInputStream(file)));
        } else {
            privateKey = getPrivateKeyFromBytes(userInfo.getPrivateKeyPemBytes().getBytes());
        }

        if (certificateFileName != null) {
            File file = new File(certificateFileName);
            certificate = new String(IOUtils.toByteArray(new FileInputStream(file)), "UTF-8");
        } else {
            certificate = userInfo.getSignedCertPemBytes();
        }

        MockUser admin = TestUtils.getMockUser(userName, mspId);
        admin.setEnrollment(TestUtils.getMockEnrollment(privateKey, certificate));
