...
public class NetworkConfigIT {
    ...
    private static void doInitialSetup() throws Exception {
        ...
        if (isNewChannel) {
            // Create the channel
            createNewChannel(FOO_CHANNEL_NAME);
        }

        final String chaincodeName = getChaincodeName();

        if (!checkInstalledChaincode(client, chaincodeName)) {

            // The chaincode we require does not exist, so deploy it...
            deployChaincode(client, FOO_CHANNEL_NAME, chaincodeName);
/*
            if (fooChannel == null) {
...
/*
    private static void deployChaincodeOnChannel(HFClient client, String channelName, String chaincodeName) throws Exception {
        Channel channel = getTheChannel(client);

        out("doInitialSetup - deploying new chaincode...");
        deployChaincode(client, channel, chaincodeName);
    }
*/
    ...
    private static ChaincodeID deployChaincode(HFClient client, String channelName, String chaincodeName) throws Exception {
    ...
    // First time creation of the channel
    private static void createNewChannel(String channelName) throws Exception {
        ...
        String ordererName = sampleOrg.getOrdererNames().iterator().next();
        Properties ordererProperties = testConfig.getOrdererProperties(ordererName);
        Orderer anOrderer = client.newOrderer(ordererName, sampleOrg.getOrdererLocation(ordererName), ordererProperties);

        ChannelConfiguration channelConfiguration = new ChannelConfiguration(new File(TEST_FIXTURES_PATH + "/sdkintegration/e2e-2Orgs/channel/" + channelName + ".tx"));

        // Create channel that has only one signer that is this org's peer admin.
        // If channel creation policy needs more signatures they would need to be added too.
        client.newChannel(channelName, anOrderer, channelConfiguration, client.getChannelConfigurationSignature(channelConfiguration, sampleOrg.getPeerAdmin()));
    ...
    private static Channel constructChannel(HFClient client, String channelName) throws Exception {
        ...
        // TODO: Should client.getChannel also initialize the channel, or are there some more things that a client
        // may need to do before initializing it?
    ...
    private static boolean checkAnyInstalledChaincode(HFClient client) throws InvalidArgumentException, ProposalException {
        ...
        Peer peer = client.getPeerFromConfig(null, Peer.PeerRole.LEDGER_QUERY);
    ...
    private static boolean checkInstalledChaincode(HFClient client, String ccName) throws InvalidArgumentException, ProposalException {

        String ccPath = CHAIN_CODE_PATH;
        String ccVersion = CHAIN_CODE_VERSION;
