...
//                    }
    ...
    CompletableFuture<BlockEvent.TransactionEvent> moveAmount(HFClient client, Channel channel, ChaincodeID chaincodeID, String moveAmount, User user) {
        ...
        try {
            ...
            transactionProposalRequest.setArgs(new byte[][] {"move".getBytes(UTF_8), //test using bytes .. could be String[] too.
    ...
    private Channel reconstructChannel(String name, HFClient client, SampleOrg sampleOrg) throws Exception {
        ...
        List<Peer> savedPeers = new ArrayList<>(newChannel.getPeers());
        ...
        newChannel.addPeer(eventingPeer, PeerOptions.create().setPeerRoles(EnumSet.of(PeerRole.EVENT_SOURCE)).startEvents(0));
        newChannel.addPeer(ledgerPeer, PeerOptions.create().setPeerRoles(EnumSet.of(PeerRole.LEDGER_QUERY)));
        ...
        final Channel finalChannel = newChannel;
        newChannel.registerBlockListener(block -> {

            try {

                final long count = bcount.getAndIncrement();
                assertEquals(count, block.getBlockNumber());
                assertEquals(eventingPeer.getName(), block.getPeer().getName());
                out("BLOCK COUNTER %d, %d  from peer: %s", count, block.getBlockNumber(), block.getPeer().getName());
                if (count == 0) {
                    ...
                    if (bcount.get() >= lh) { // test with latest count.
        ...
        assertEquals(newChannel.queryBlockchainInfo().getHeight(), bcount.get()); //finalcount is zero based.
        newChannel.shutdown(true);
...
//        ArrayList<Peer> saved = new ArrayList();
//
//        //Play with adding removing peers.
//        for (Peer peer : new ArrayList<Peer>(newChannel.getPeers(EnumSet.of(PeerRole.EVENT_SOURCE, PeerRole.ENDORSING_PEER)))) {
//            newChannel.removePeer(peer);
//            assertFalse(saved.contains(peer));
//            newChannel.g
//                    asse
//            saved.add(peer);
//        }
//
//        assertFalse(saved.isEmpty()); // must be some endorsing peers.
//        assertTrue(newChannel.getPeers(EnumSet.of(PeerRole.CHAINCODE_QUERY, PeerRole.ENDORSING_PEER)).isEmpty());
//
//        saved.forEach((peer) -> {
...
//        });
    ...
    private void queryChaincodeForExpectedValue(HFClient client, Channel channel, final String expect, ChaincodeID chaincodeID) {
        ...
        queryByChaincodeRequest.setArgs("query".getBytes(UTF_8), "b".getBytes(UTF_8)); // test using bytes as args.
