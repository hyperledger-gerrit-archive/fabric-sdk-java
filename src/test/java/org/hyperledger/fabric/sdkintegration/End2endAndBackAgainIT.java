...
//            //SampleUser can be any implementation that implements org.hyperledger.fabric.sdk.User Interface
            ...
            if (!testConfig.isRunningAgainstFabric10()) { //Peer eventing service support started with v1.1
                ...
                testPeerServiceEventingReplay(client, replayChannel, 0L);

                //Now do it again starting at block 1
                replayChannel = client.deSerializeChannel(replayChannelBytes);
                testPeerServiceEventingReplay(client, replayChannel, 1L);
            }
...
//                    }
    ...
    void testPeerServiceEventingReplay(HFClient client, Channel replayTestChannel, long start) throws InvalidArgumentException, TransactionException, InterruptedException, ExecutionException, TimeoutException, ProposalException {
        ...
        final AtomicLong height = new AtomicLong(Long.MAX_VALUE);
        final Channel finalChannel = replayTestChannel;
        final AtomicBoolean gotBlockZero = new AtomicBoolean(false);

        final String blockListenerHandle = replayTestChannel.registerBlockListener(block -> { // register a block listener

            try {

                final long count = bcount.getAndIncrement(); //count starts with 0 not 1 !
                assertEquals(format("count: %d, start: %d, blockNumber: %d", count, start, block.getBlockNumber()),
                        count + start, block.getBlockNumber());
                assertEquals(eventingPeer.getName(), block.getPeer().getName());
                out("Block count: %d, block number: %d  received from peer: %s", count, block.getBlockNumber(), block.getPeer().getName());
                if (block.getBlockNumber() == start) {
                    // for this let check we got it once.
                    assertFalse("Seems we seen block zero twice", gotBlockZero.getAndSet(true));
                ...
                if (count == 0) {
                    ...
                    height.set(lh);
                    out("height: %d", height.get());
                    if (bcount.get() + start >= lh) { // test with latest count.
        ...
        assertTrue("Block height not greater than zero was:" + bcount.toString(), bcount.get() > 0L); // make sure we got one at least
        assertEquals(replayTestChannel.queryBlockchainInfo().getHeight(), bcount.get() + start); //finalcount is zero based.
        assertTrue("We didn't get block number 0", gotBlockZero.get()); // make sure block zero was detected.
