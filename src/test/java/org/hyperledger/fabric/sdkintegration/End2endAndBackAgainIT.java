...
//                    }
    ...
    void testPeerServiceEventingReplay(HFClient client, Channel replayTestChannel, long start, long stop) throws InvalidArgumentException, TransactionException, InterruptedException, ExecutionException, TimeoutException, ProposalException {
        ...
        final AtomicLong height = new AtomicLong(stop == -1L ? Long.MAX_VALUE : stop);
        ...
        for (long i = height.longValue() - 1L; i >= start; i--) { //make sure all are there.
