...
//        assertEquals(256, Config.getConfig().getSecurityLevel());
    ...
    public void setup() {
        ...
        try {
            ...
            assertTrue(eventCount > 0); // make sure we got blockevent that were tested.
    ...
    private Channel reconstructChannel(String name, HFClient client, SampleOrg sampleOrg) throws Exception {
        ...
        newChannel.registerBlockListener(blockEvent -> {
            ...
            assertTrue(format("nontransactions %d, transactions %d", nonTransactions, transactions), nonTransactions < 2); // not transaction8 blocks only have one envelope
            ...
            if (nonTransactions > 0) { // this is an update block -- don't care about others here.
                ...
                if (blockEvent.isFiltered()) {
                    ++eventCount; // make sure we're seeing non transaction events.
                } else {
                    ++eventCountBlock;
                }
