    ...
    void blockWalker(Chain chain) throws InvalidProtocolBufferException, InvalidArgumentException, ProposalException, UnsupportedEncodingException {
        ...
        try {
            ...
            for (long current = channelInfo.getHeight() - 1; current > -1; --current) {
                ...
                out("current block number %d has previous has id: %s", blockNumber, Hex.encodeHexString(returnedBlock.getPreviousHash()));
                out("current block number %d has %d envelopes", blockNumber, returnedBlock.getEnvelopCount());
                int i = 0;
