...
public class End2endIT {
    ...
    private Hashtable<String, String> MSPIDs = new Hashtable<>();
    private Hashtable<String, SampleUser> admins = new Hashtable<>();
    private Hashtable<String, Set<String>> peers = new Hashtable<>();
    private Hashtable<String, Set<Peer>> orgPeers = new Hashtable<>();
    private Hashtable<String, SampleUser> users = new Hashtable<>();
    private Hashtable<String, HFCAClient> fabricCAs = new Hashtable<>();

    String testTxID = null ;  // save the CC invoke TxID and use in queries

    private final Collection<String> PEER_LOCATIONS = Arrays.asList(testConfig.getIntegrationTestsPeers().split(","));

    private final Collection<String> MSPS = Arrays.asList(testConfig.getIntegrationTestsMSPIDs().split(","));

    private final Collection<String> ORDERER_LOCATIONS = Arrays.asList(testConfig.getIntegrationTestsOrderers().split(","));

    private final Collection<String> EVENTHUB_LOCATIONS = Arrays.asList(testConfig.getIntegrationtestsEventhubs().split(","));

    private final String[] FABRIC_CA_SERVICES_LOCATION = testConfig.getIntegrationtestsFabricCA().split(",");
    ...
    public void setup() {
        ...
        try {
            ...
            runChain(client, constructChain(FOO_CHAIN_NAME, client, "peerOrg1"), true, "peerOrg1", 0);
            out("\n");
            runChain(client, constructChain(BAR_CHAIN_NAME, client, "peerOrg2"), false, "peerOrg2", 100); //run a newly constructed foo chain with different b value!
    ...
    void runChain(HFClient client, Chain chain, boolean installChainCode, String orgToUse, int delta) {
        ...
        try {
            ...
            */
            ChaincodeEndorsementPolicy chaincodeEndorsementPolicy = new ChaincodeEndorsementPolicy(new File("src/test/fixture/e2e-2Orgs/channel/members_from_org1_or_2.policy"));
            ...
            Set<Peer> peerSet = orgPeers.get(orgToUse);
    ...
    private Chain constructChain(String  name, HFClient client, String org) throws Exception {
        ...
        for (String anOrg : orgs) {
            ...
            for (String peerloc : peersUrl) {
                String[] temp = peerloc.split("@");
                Peer peer = client.newPeer(temp[1]);
                peer.setName(temp[0]);
