...
public class ServiceDiscoveryIT {
    ...
    public void setup() throws Exception {
        ...
        Properties properties = testConfig.getEndPointProperties("peer", "peer0.org1.example.com");
        ...
        final Properties peerProps = new Properties(); // running with mutual auth need client keys that are signed by appropriate CA .. map by mspid
        peerProps.put("peer.Org1MSP.certfile", "src/test/fixture/sdkintegration/e2e-2Orgs/v1.1/crypto-config/peerOrganizations/org1.example.com/users/User1@org1.example.com/tls/client.crt");
        peerProps.put("peer.Org1MSP.keyfile", "src/test/fixture/sdkintegration/e2e-2Orgs/v1.1/crypto-config/peerOrganizations/org1.example.com/users/User1@org1.example.com/tls/client.key");
        peerProps.put("peer.Org2MSP.certfile", "src/test/fixture/sdkintegration/e2e-2Orgs/v1.1/crypto-config/peerOrganizations/org2.example.com/users/User1@org2.example.com/tls/client.crt");
        peerProps.put("peer.Org2MSP.keyfile", "src/test/fixture/sdkintegration/e2e-2Orgs/v1.1/crypto-config/peerOrganizations/org2.example.com/users/User1@org2.example.com/tls/client.key");
        ...
        foo.setSDPeerAddition(sdPeerAdditionInfo -> {
            ...
            if (null == peer) {
                ...
                String prop = peerProps.getProperty("peer." + mspid + ".certfile");

                peerProperties.put("clientCertFile", prop);

                prop = peerProps.getProperty("peer." + mspid + ".keyfile");
                peerProperties.put("clientKeyFile", prop);

                byte[][] tlsCerts = sdPeerAdditionInfo.getTlsCerts();
                ...
                tlsCerts = sdPeerAdditionInfo.getTlsIntermediateCerts();
                ...
                if (bytes.length > 0) {
                    peerProperties.put("pemBytes", bytes);
                }
                ...
                peer = sdPeerAdditionInfo.getClient().newPeer(sdPeerAdditionInfo.getEndpoint(),
                        (testConfig.isRunningFabricTLS() ? "grpcs" : "grpc") + "://" + newEndpoint,
                        peerProperties);
            ...
            return peer; // the peer added.

        });

        //Application override default orderer creation for service discovery.
        ...
        foo.setSDOrdererAddition(sdOrdererAdditionInfo -> {
            ...
            if (null == orderer) {
                ...
                //Need client keys/certs signed by appropriate CA
                ...
                byte[][] tlsCerts = sdOrdererAdditionInfo.getTlsCerts();
                ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
                for (byte[] tlsCert : tlsCerts) {
                    try {
                        outputStream.write(tlsCert);
                ...
                tlsCerts = sdOrdererAdditionInfo.getTlsIntermediateCerts();
                ...
                if (bytes.length > 0) {
                    ordererProperties.put("pemBytes", bytes);
                }
