...
//        }
    ...
    void runChannel(HFClient org1Client, Channel org1Channel, SampleOrg org1, Collection<Peer> org1MyPeers, Collection<Peer> org1OtherPeers,
        ...
        try {
            ...
            String org1ChaincodePackageID = installChaincode(org1Client, org1MyPeers, lifecycleChaincodePackage);
            ...
            verifyInstalledChaincodes(org1Client, org1MyPeers, CHAINCODE_LABEL, org1ChaincodePackageID);
            // another query test if it works
            verifyInstalledChaincode(org1Client, org1MyPeers, org1ChaincodePackageID, CHAINCODE_LABEL);
...
//            }
            ...
            verifyApprovedViaQueryApprovalStatus(org1Client, org1Channel, org1MyPeers, definitionSequence, CHAIN_CODE_NAME, CHAIN_CODE_VERSION,
            ...
            TransactionEvent transactionEvent = lifecycleApproveChaincodeDefinitionForMyOrg(org1Client, org1Channel,
                    ...
                    definitionSequence, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, chaincodeEndorsementPolicy, initRequired, org1ChaincodePackageID)
            ...
            String org2ChaincodePackageID = installChaincode(org2Client, org2MyPeers, org2LifecycleChaincodePackage);
            //compare the hashes just for testing.
            // assertEquals(org1ChaincodePackageID, org2ChaincodePackageID);
            ...
            verifyInstalledChaincodes(org2Client, org2MyPeers, CHAINCODE_LABEL, org2ChaincodePackageID);
            // check by querying for specific chaincode
            verifyInstalledChaincode(org2Client, org2MyPeers, org2ChaincodePackageID, CHAINCODE_LABEL);
            ...
            TransactionEvent org2TransactionEvent = lifecycleApproveChaincodeDefinitionForMyOrg(org2Client, org2Channel,
                    ...
                    definitionSequence, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, org2ChaincodeEndorsementPolicy, initRequired, org2ChaincodePackageID)
                    .get(testConfig.getTransactionWaitTime(), TimeUnit.SECONDS);
            assertTrue(org2TransactionEvent.isValid());

            out("Checking on org2's network for approvals");
            verifyApprovedViaQueryApprovalStatus(org2Client, org2Channel, org2MyPeers, definitionSequence, CHAIN_CODE_NAME, CHAIN_CODE_VERSION,
                    initRequired, chaincodeEndorsementPolicy,
    ...
    private void verifyApprovedViaQueryApprovalStatus(HFClient client, Channel channel, Collection<Peer> org1MyPeers, long definitionSequence,
                                                      String chaincodeName, String chaincodeVersion, boolean initRequired,
    ...
    CompletableFuture<TransactionEvent> lifecycleApproveChaincodeDefinitionForMyOrg(HFClient client, Channel channel,
                                                                                    Collection<Peer> peers, long definitionSequence, String chaincodeName, String chaincodeVersion, ChaincodeEndorsementPolicy chaincodeEndorsementPolicy, boolean initRequired, String org1ChaincodePackageID) throws InvalidArgumentException, ProposalException {
    ...
    private String installChaincode(HFClient client, Collection<Peer> peers, LifecycleChaincodePackage lifecycleChaincodePackage) throws InvalidArgumentException, ProposalException, InvalidProtocolBufferException {
        ...
        for (LifecycleInstallProposalResponse response : responses) {
            if (response.getStatus() == ProposalResponse.Status.SUCCESS) {
                ...
                successful.add(response);
                if (ch == null) {
                    ch = response.getPackageId();
                    assertNotNull(format("Hashcode came back as null from peer: %s ", response.getPeer()), ch);
                } else {
                    assertEquals("Miss match on what the peers returned back as the packageID", ch, response.getPackageId());
                }
            } else {
                failed.add(response);
            }
    ...
    private void verifyInstalledChaincode(HFClient client, Collection<Peer> peers, String packageId, String expectedLabel) throws ProposalException, InvalidArgumentException {

        Collection<LifecycleQueryInstalledChaincodeProposalResponse> chaincodeResponces = client.lifecycleQueryInstalledChaincode(packageId, peers);
        assertNotNull(chaincodeResponces);
        assertEquals("responses not same as peers", peers.size(), chaincodeResponces.size());
        ...
        for (LifecycleQueryInstalledChaincodeProposalResponse ic : chaincodeResponces) {
            ...
            assertEquals(format("Peer %s returned back different label", peerName), expectedLabel, ic.getLabel());
...
//    }
    ...
    private void verifyInstalledChaincodes(HFClient client, Collection<Peer> peers, String chaincodeLabel, String packageId) throws ProposalException, InvalidArgumentException {
        ...
        for (LifecycleQueryInstalledChaincodesProposalResponse peerResults : results) {
            ...
            for (LifecycleQueryInstalledChaincodesResult lifecycleQueryInstalledChaincodesResult : peerResults.getLifecycleQueryInstalledChaincodesResult()) {
                ...
                if (packageId.equals(lifecycleQueryInstalledChaincodesResult.getPackageId())) {
                    assertEquals(format("Peer %s had chaincode lable mismatch", peerName), chaincodeLabel, lifecycleQueryInstalledChaincodesResult.getLabel());
                    found = true;
                    break;
                }

            }
            assertTrue(format("Chaincode label %s, packageId %s not found on peer %s ", chaincodeLabel, packageId, peerName), found);
