...
//        }
    ...
    void runChannel(HFClient org1Client, Channel org1Channel, SampleOrg org1, Collection<Peer> org1MyPeers, Collection<Peer> org1OtherPeers,
        ...
        try {
            ...
            byte[] org1ChaincodeHash = installChaincode(org1Client, org1MyPeers, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, lifecycleChaincodePackage);

            assertNotNull(org1ChaincodeHash);
            assertEquals(32, org1ChaincodeHash.length);
            //out("hash: %s", Utils.toHexString(org1ChaincodeHash));
            ...
            verifyInstalledChaincodes(org1Client, org1MyPeers, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, org1ChaincodeHash);
            // another query test if it works
            verifyInstalledChaincode(org1Client, org1MyPeers, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, org1ChaincodeHash);

            //Org1 also creates the endorsement policy for the chaincode.
            ChaincodeEndorsementPolicy chaincodeEndorsementPolicy = ChaincodeEndorsementPolicy.fromYamlFile(Paths.get(TEST_FIXTURES_PATH + "/sdkintegration/chaincodeendorsementpolicy.yaml"));

            //Org1 approves the chaincode to run for it's peers.
            ...
            TransactionEvent transactionEvent = lifecycleApproveChaincodeDefinitionForMyOrg(org1Client, org1Channel,
                    Collections.singleton(org1MyPeers.iterator().next()),  //support approve on multiple peers but really today only need one. Go with minimum.
                    definitionSequence, org1ChaincodeHash, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, chaincodeEndorsementPolicy, initRequired)
            ...
            byte[] chaincodeEndorsementPolicyAsBytes = chaincodeEndorsementPolicy.getChaincodeEndorsementPolicyAsBytes();
            ...
            ChaincodeEndorsementPolicy org2ChaincodeEndorsementPolicy = ChaincodeEndorsementPolicy.fromBytes(chaincodeEndorsementPolicyAsBytes);
            ...
            byte[] org2ChaincodeHash = installChaincode(org2Client, org2MyPeers, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, org2LifecycleChaincodePackage);
            //compare the hashes just for testing.
            assertArrayEquals(org1ChaincodeHash, org2ChaincodeHash);
            ...
            verifyInstalledChaincodes(org2Client, org2MyPeers, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, org2ChaincodeHash);
            // check by querying for specific chaincode
            verifyInstalledChaincode(org2Client, org2MyPeers, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, org2ChaincodeHash);
            ...
            TransactionEvent org2TransactionEvent = lifecycleApproveChaincodeDefinitionForMyOrg(org2Client, org2Channel,
                    ...
                    definitionSequence, org2ChaincodeHash, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, org2ChaincodeEndorsementPolicy, initRequired)
                    .get(testConfig.getTransactionWaitTime(), TimeUnit.SECONDS);
            ...
            out("Org 2 doing commmit chaincode definition");
            Collection<Peer> org2EndorsingPeers = Arrays.asList(org2MyPeers.iterator().next(), org2OtherPeers.iterator().next());
            transactionEvent = commitChaincodeDefinitionRequest(org2Client, org2Channel, definitionSequence, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, org2ChaincodeHash, org2ChaincodeEndorsementPolicy, org2EndorsingPeers, initRequired)
                    .get(testConfig.getTransactionWaitTime(), TimeUnit.SECONDS);
            assertTrue(transactionEvent.isValid());
            ...
            //query definitions
            Collection<LifecycleQueryChaincodeDefinitionProposalResponse> queryChaincodeDefinitionProposalResponses = org2Channel.lifecycleQueryChaincodeDefinition(CHAIN_CODE_NAME, org2MyPeers);

            assertNotNull(queryChaincodeDefinitionProposalResponses);
            assertEquals(org2MyPeers.size(), queryChaincodeDefinitionProposalResponses.size());
            for (LifecycleQueryChaincodeDefinitionProposalResponse response : queryChaincodeDefinitionProposalResponses) {
                ...
                assertEquals(definitionSequence, response.getSequence());
                assertArrayEquals(org2ChaincodeHash, response.getHash());
                ChaincodeEndorsementPolicy chaincodeEndorsementPolicy1 = response.getChaincodeEndorsementPolicy();
                CollectionConfigPackage collections = response.getCollections();
                assertEquals(initRequired, response.getInitRequired());
                String endorsementPlugin = response.getEndorsementPlugin();
                String validationPlugin = response.getValidationPlugin();

            }

            verifyCommitedChaincodeByNamespaceQuery(org2Channel, org2MyPeers, CHAIN_CODE_NAME);
            verifyCommitedChaincodeByNamespaceQuery(org1Channel, org1MyPeers, CHAIN_CODE_NAME);

            //Now org2 could also do the init for the chaincode but it just informs org2 admin of the commit so it does it.
            out("Org1 doing init");
            transactionEvent = executeChaincode(org1Client, org1.getPeerAdmin(), org1Channel, "init", "a,", "100", "b", "300").get(testConfig.getTransactionWaitTime(), TimeUnit.SECONDS);
            assertTrue(transactionEvent.isValid());

            transactionEvent = executeChaincode(org2Client, org2.getPeerAdmin(), org2Channel, "move", "a,", "b", "10").get(testConfig.getTransactionWaitTime(), TimeUnit.SECONDS);
            assertTrue(transactionEvent.isValid());

            transactionEvent = executeChaincode(org1Client, org1.getPeerAdmin(), org1Channel, "move", "a,", "b", "10").get(testConfig.getTransactionWaitTime(), TimeUnit.SECONDS);
            assertTrue(transactionEvent.isValid());
    ...
    private void verifyCommitedChaincodeByNamespaceQuery(Channel channel, Collection<Peer> peers, String expectChaincodeName) throws InvalidArgumentException, ProposalException {
    ...
    CompletableFuture<TransactionEvent> executeChaincode(HFClient client, User userContext, Channel channel, String fcn, String... args) throws InvalidArgumentException, ProposalException {
    ...
    CompletableFuture<TransactionEvent> lifecycleApproveChaincodeDefinitionForMyOrg(HFClient client, Channel channel, Collection<Peer> peers, long definitionSequence, byte[] chaincodeHash, String chaincodeName, String chaincodeVersion, ChaincodeEndorsementPolicy chaincodeEndorsementPolicy, boolean initRequired) throws InvalidArgumentException, ProposalException, InterruptedException, ExecutionException, TimeoutException {
        ...
        lifecycleApproveChaincodeDefinitionForMyOrgRequest.setChaincodeHash(chaincodeHash);
        ...
        for (LifecycleApproveChaincodeDefinitionForMyOrgProposalResponse response : lifecycleApproveChaincodeDefinitionForMyOrgProposalResponse) {
            assertFalse(response.getMessage(), response.isInvalid());
    ...
    private byte[] installChaincode(HFClient client, Collection<Peer> peers, String chaincodeName, String chaincodeVersion, LifecycleChaincodePackage lifecycleChaincodePackage) throws InvalidArgumentException, ProposalException, InvalidProtocolBufferException {

        LifecycleInstallRequest installProposalRequest = client.newLifecycleInstallRequest();
        ...
        for (LifecycleInstallProposalResponse response : responses) {
            if (response.getStatus() == ProposalResponse.Status.SUCCESS) {
                ...
                successful.add(response);
                if (ch == null) {
                    ch = response.getChaincodeHash();
                    assertNotNull(format("Hashcode came back as null from peer: %s ", response.getPeer()), ch);
                } else {
                    assertArrayEquals("Miss match on what the peers returned back as the hash", ch, response.getChaincodeHash());
                }
            } else {
                failed.add(response);
            }
    ...
    private void verifyInstalledChaincode(HFClient client, Collection<Peer> peers, String chaincodeName, String chaincodeVersion, byte[] chaincodeHash) throws ProposalException, InvalidArgumentException {

        Collection<LifecycleQueryInstalledChaincodeProposalResponse> chaincodeResponces = client.lifecycleQueryInstalledChaincode(chaincodeName, chaincodeVersion, peers);
        assertNotNull(chaincodeResponces);
        assertEquals("responces not same as peers", peers.size(), chaincodeResponces.size());

        for (LifecycleQueryInstalledChaincodeProposalResponse ic : chaincodeResponces) {
            String peerName = ic.getPeer().getName();
            assertEquals(format("Peer %s returned back bad status code", peerName), ChaincodeResponse.Status.SUCCESS, ic.getStatus());

            assertArrayEquals(format("Peer %s had mismatch with expected hash", peerName), chaincodeHash, ic.getChaincodeHash());
        }
    }

    /**
     * Check that the chaincode name/version is not installed on the peer.
     *
     * @param client
     * @param peers
     * @param chaincodeName
     ...
     */
    private void verifyNotInstalledChaincode(HFClient client, Collection<Peer> peers, String chaincodeName, String chaincodeVersion) throws ProposalException, InvalidArgumentException {

        Collection<LifecycleQueryInstalledChaincodeProposalResponse> chaincodeResponces = client.lifecycleQueryInstalledChaincode(chaincodeName, chaincodeVersion, peers);
        assertNotNull(chaincodeResponces);
        assertEquals("Response number not same as peers", peers.size(), chaincodeResponces.size());

        for (LifecycleQueryInstalledChaincodeProposalResponse ic : chaincodeResponces) {
            String peerName = ic.getPeer().getName();
            assertEquals(format("Peer %s returned back success for chaincode should not be there!", peerName), ChaincodeResponse.Status.FAILURE, ic.getStatus());
        }
    }

    private void verifyInstalledChaincodes(HFClient client, Collection<Peer> peers, String chaincodeName, String chaincodeVersion, byte[] chaincodeHash) throws ProposalException, InvalidArgumentException {

        Collection<LifecycleQueryInstalledChaincodesProposalResponse> results = client.lifecycleQueryInstalledChaincodes(peers);
        ...
        for (LifecycleQueryInstalledChaincodesProposalResponse result : results) {
            ...
            for (LifecycleQueryInstalledChaincodesResult lifecycleQueryInstalledChaincodesResult : result.getLifecycleQueryInstalledChaincodesResult()) {

                if (chaincodeName.equals(lifecycleQueryInstalledChaincodesResult.getName()) && chaincodeVersion.equals(chaincodeVersion)) {
                    assertArrayEquals(format("Peer %s had chaincode hash mismatch", peerName), chaincodeHash, lifecycleQueryInstalledChaincodesResult.getHash());
                    found = true;
                    break;
                }

            }
            assertTrue(format("Chaincode %s, version %s not found on peer %s ", chaincodeName, chaincodeHash, peerName), found);
    ...
    private CompletableFuture<TransactionEvent> commitChaincodeDefinitionRequest(HFClient client, Channel channel, long definitionSequence, String chaincodeName, String chaincodeVersion,
                                                                                 byte[] chaincodeHash, ChaincodeEndorsementPolicy chaincodeEndorsementPolicy, Collection<Peer> endorsingPeers, boolean initRequired) throws ProposalException, InvalidArgumentException, InterruptedException, ExecutionException, TimeoutException {
        ...
        lifecycleCommitChaincodeDefinitionRequest.setChaincodeHash(chaincodeHash);
