...
//        }
    ...
    public void runFabricTest() throws Exception {
        ...
        Collection<Peer> og1MyPeers = org1Channel.getPeers();
        ...
        runChannel(org1Client, org1Channel, org1, og1MyPeers, org1OtherPeers,
    ...
    void runChannel(HFClient org1Client, Channel org1Channel, SampleOrg org1, Collection<Peer> org1MyPeers, Collection<Peer> org1OtherPeers,
        ...
        try {
            ...
            verifyInstalledChaincodes(org1Client, org1Channel, org1MyPeers, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, org1ChaincodeHash);
            ...
            verifyInstalledChaincodes(org2Client, org2Channel, org2MyPeers, CHAIN_CODE_NAME, CHAIN_CODE_VERSION, org2ChaincodeHash);
    ...
    private byte[] installChaincode(HFClient client, Channel channel, Collection<Peer> peers, String chaincodeName, String chaincodeVersion, LifecycleChaincodePackage lifecycleChaincodePackage) throws InvalidArgumentException, ProposalException, InvalidProtocolBufferException {
        ...
        LifecycleInstallProposalResponse lifecycleInstallProposalResponse = responses.iterator().next();

        return lifecycleInstallProposalResponse.getChaincodeHash();

    }

    private void verifyInstalledChaincodes(HFClient client, Channel channel, Collection<Peer> peers, String chaincodeName, String chaincodeVersion, byte[] chaincodeHash) throws ProposalException, InvalidArgumentException {
        ...
        for (Peer peer : peers) {
            ...
            for (Lifecycle.QueryInstalledChaincodesResult.InstalledChaincode ic : chaincodeInfos) {
                if (chaincodeName.equals(ic.getName()) && chaincodeVersion.equals(ic.getVersion())) {
                    assertArrayEquals(format("Channel %s Chaincode %s unexpected hash peer %s", channel, peer, chaincodeName), ic.getHash().toByteArray(), chaincodeHash);
                    found = true;
                    break;
            ...
            assertTrue(format("Channel %s Chaincode %s not found on peer %s", channel, peer, chaincodeName), found);
